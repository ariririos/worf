...
impl Decoder for FFmpegDecoder {
    fn decode(path: &Path) -> BlissResult<PreAnalyzedSong> {
        ...
        if let Some(artist) = context_or_stream.metadata().get("artist") {
            song.artist = match artist {
                "" => None,
                a => Some(a.to_string()),
            };
        };
        if let Some(album) = context_or_stream.metadata().get("album") {
            song.album = match album {
                "" => None,
                a => Some(a.to_string()),
            };
        };
        if let Some(genre) = context_or_stream.metadata().get("genre") {
            song.genre = match genre {
                "" => None,
                g => Some(g.to_string()),
            };
        };
        if let Some(track_number) = context_or_stream.metadata().get("track") {
        ...
        if let Some(disc_number) = context_or_stream.metadata().get("disc") {
        ...
        if let Some(album_artist) = context_or_stream.metadata().get("album_artist") {
